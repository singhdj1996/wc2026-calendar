...
FIFA_SCHEDULE_ARTICLE_URL = "https://www.fifa.com/en/tournaments/mens/worldcup/canadamexicousa2026/articles/match-schedule-fixtures-results-teams-stadiums"

TZ = ZoneInfo("Europe/Berlin")
CALENDAR_NAME = "FIFA World Cup 2026 - ARD/ZDF"
INCLUDE_UNRESOLVED = False

# Keep this off unless you explicitly want the old morning filter again.
EXCLUDE_01_TO_12 = False

@dataclass
class Match:
    match_no: int
    date: str                  # YYYY-MM-DD in Europe/Berlin viewing date
    time: str                  # HH:MM in Europe/Berlin viewing time
    round: str
    group: str
    team_a: str
    team_b: str
    ...
    source_url: str
    notes: str = ""
    include: str = ""          # override: yes/no/blank
...
def normalize_stream(value: str) -> str:
    ...
    if "ZDF" in upper:
        return "ZDF"
    if "TBC" in upper or "UNKNOWN" in upper or not text:
        return "TBC"
    return text

def is_ard_zdf(stream: str) -> bool:
    norm = normalize_stream(stream)
    return norm in {"ARD", "ZDF", "ARD/ZDF"}
...
def maybe_parse_date_time(value: str) -> tuple[str, str] | None:
    """Return Europe/Berlin date/time from a guessed FIFA datetime string."""
    text = clean(value)
    if not text:
        return None
    ...
    for cand in candidates:
        try:
            ...
            if dt.tzinfo is None:
                # If timezone is missing, assume FIFA value is already local-like; not ideal.
    ...
    # YYYY-MM-DD + HH:MM somewhere in a string
...
def extract_json_candidates(html_text: str) -> list[object]:
    ...
    # JSON-LD or generic application/json scripts
    ...
    # Next app-router often embeds self.__next_f.push([...])
    for body in re.findall(r"self\.__next_f\.push\((.*?)\)</script>", html_text, re.DOTALL):
        try:
            parsed = json.loads(body)
            candidates.append(parsed)
        except json.JSONDecodeError:
            # Sometimes JSON is nested as escaped strings; keep rough text fallback.
            pass
...
def object_to_match(d: dict) -> Match | None:
    ...
    if not match_no_raw:
        # Some FIFA objects use id, but avoid treating any id as match number.
        return None
    ...
    # Sometimes teams are nested deeper or named differently.
    if not home:
...
def fetch_fifa_matches() -> list[Match]:
    """Best-effort FIFA parser. Falls back safely if FIFA frontend changes."""
    headers = {
...
def apply_overrides(matches: list[Match]) -> list[Match]:
    ...
    for row in read_csv(OVERRIDES_CSV):
        ...
        existing.source_url = clean(row.get("source_url")) or existing.source_url
...
def included_matches(matches: list[Match]) -> tuple[list[Match], list[dict]]:
    included: list[Match] = []
    audit: list[dict] = []
...
def event_duration(m: Match) -> timedelta:
    round_lower = m.round.lower()
    if any(k in round_lower for k in ["round of", "quarter", "semi", "final", "third"]):
...
def write_status(included: list[Match], audit: list[dict], source_used: str) -> None:
    status = {
...
def write_index(included: list[Match], audit: list[dict], source_used: str) -> None:
    rows = "\n".join(
...
<body>
  ...
  <h2>Debug</h2>
  <p>Full generation status: <a href="status.json">status.json</a></p>
</body>
</html>
"""
...
def main() -> int:
    ...
    included, audit = included_matches(matches)

    write_ics(included)
    write_status(included, audit, source_used)
    write_index(included, audit, source_used)

    print(f"Source used: {source_used}")
    print(f"Total matches after source + overrides: {len(matches)}")
    print(f"Included ARD/ZDF resolved matches: {len(included)}")
